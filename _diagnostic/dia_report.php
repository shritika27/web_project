<?php
	...
	if(isset($_REQUEST['patient_id'])){
		...
		$dia_sql = "select * from daignostic_center where id = '$_REQUEST[d_id]'";
		$dia_rs = mysqli_query($conn, $dia_sql);
		$data1 = mysqli_fetch_assoc($dia_rs);
		// print "<pre>";
		// print_r($data1);
	}
			...
			<div class="report_banner">
						...
						<div class="col-md-8 header_top">
							...
							<div class="row">
								...
								<div class="col-md-6 dc_dt">
										<div class="hd">
											<h5>Diagnostic Details</h5>
										</div>
										<div class="">
											<div><label>Diagnostic Name:</label><span><?php echo $data1['dia_name']; ?></span></div>
											<div><label>Diagnostic Location:</label><span><?php echo $data1['dia_loc']; ?></span></div>
											<div><label>Diagnostic Contact No:</label><span><?php echo $data1['dia_contnum']; ?></span></div>
										</div>
								</div>
				...
				<div class="row">
					<div class="col-md-12">
						...
						<input type="hidden" name="dia_id" value="<?php if(isset($data1['id'])){ echo $data1['id']; }?>" />
